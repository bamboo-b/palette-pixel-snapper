...
上のURLを開くだけ。**画像を入れれば自動で変換され、あとは保存するだけ**でドット絵が整います。
...
2. **自動でドット絵に変換されます**（ボタン操作は不要）
3. before/after を見比べて、「**画像を保存（PNG）**」で書き出す

調整したい人は、左の「**⚙️ 設定**」パネルへ（どちらのタブも使わなくてもOK）。

- 「**色数・効果**」タブ（上のスクリーンショットに表示）：色数の指定（2〜64色）、色選びを引き直すseed、ディザリングのON/OFF。
- 「**パレット**」タブ：使う色の貼り付けや、パレットファイル（`.hex` / `.gpl` / `.pal` / 画像）の**読み込み・ドラッグ&ドロップ**、暖色・寒色などによる色の絞り込み。

<img src="./static/gui-palette.png" alt="パレットタブでPICO-8パレットを読み込み、その色だけで変換した画面" style="width: 100%;">

<p align="center"><em>パレットタブでPICO-8パレットを読み込んだ例。変換結果がパレットの色だけで塗り直されます。</em></p>
